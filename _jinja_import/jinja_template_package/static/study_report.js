(function () {
  ...
  const controls = Array.from(document.querySelectorAll("input[id][name], textarea[id][name]"))
    .filter((control) => control.name !== "csrf_token");
  ...
  function narrative2() {
    ...
    return `<aside class="section-narrative functional-summary">${paragraphs.map((p) => `<p>${p}</p>`).join("")}<cite>(Weber KT, Janicki JS. Exercise Testing in the Evaluation of the Patient with Chronic Cardiac Failure. Am Rev Respir Dis. 1984; 129: S60–S62)</cite></aside>`;
  }
  ...
  function narrative3() {
    ...
    return paragraphs.length ? `<aside class="section-narrative cardiovascular-summary">${paragraphs.map((p) => `<p>${p}</p>`).join("")}</aside>` : "";
  }
  ...
  function narrative4() {
    const ratio = (id) => {
      ...
      return escapeHtml(Number.isFinite(numeric) && numeric > 1 ? (numeric / 100).toFixed(2) : raw);
    };
    ...
    if (any(ventilation)) {
      ...
      if (has("gx_vo2_max_ve_per_mvv_pct")) line += `La relación VE/VVM fue de <em>${ratio("gx_vo2_max_ve_per_mvv_pct")}</em>`;
      if (has("gx_vo2_max_vt_per_ic_pct")) line += `${has("gx_vo2_max_ve_per_mvv_pct") ? " y la relación" : "La relación"} VT/CI fue de <em>${ratio("gx_vo2_max_vt_per_ic_pct")}</em>`;
    ...
    return paragraphs.length ? `<aside class="section-narrative ventilatory-summary">${paragraphs.map((p) => `<p>${p}</p>`).join("")}</aside>` : "";
  }
  ...
  document.getElementById("toggle-detail")?.addEventListener("click", (event) => {
    ...
    document.body.classList.toggle("detail-visible", visible);
